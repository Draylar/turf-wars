...
public class TurfWarsIdle {
    ...
    public void onOpen(long time, TurfWarsConfig config) {
    ...
    private void tickStartWaiting(long time, GameSpace world) {
        ...
        if ((this.startTime - time) % 20 == 0) {
            if (sec > 0) {
                TurfWarsActive.broadcastTitle(new LiteralText(Integer.toString(sec)).formatted(Formatting.BOLD), world);
                TurfWarsActive.broadcastSound(SoundEvents.BLOCK_NOTE_BLOCK_HARP, 1.0F, world);
            } else {
                TurfWarsActive.broadcastTitle(new LiteralText("Go!").formatted(Formatting.BOLD), world);
                TurfWarsActive.broadcastSound(SoundEvents.BLOCK_NOTE_BLOCK_HARP, 2.0F, world);
            }
        }
    }
