...
public final class TurfWarsTimerBar implements AutoCloseable {

    private final ServerBossBar bar;
    private String prefix = "";
    private boolean reverse = false;

    public TurfWarsTimerBar() {
    ...
    public void update(long ticksUntilEnd, long totalTicksUntilEnd) {
        if (ticksUntilEnd % 20 == 0) {
            if(reverse) {
                this.bar.setName(this.getText(totalTicksUntilEnd - ticksUntilEnd));
            } else {
    ...
    private Text getText(long ticksUntilEnd) {
        ...
        String time = String.format(prefix + "%02d:%02d left", minutes, seconds);
